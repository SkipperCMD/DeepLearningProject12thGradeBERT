...
class FindingWordo(App):
    ...
    def build(self): # Constructor function which configures and defines the application's GUI, functionalities, etc.
        ...
        ##################################
        # Using the pre-trained BERT model for now as a testing for the app.
        # Once the app is in working order, I will replace the pre-trained model with my own model which I have trained.
        ##################################
        ...
        def FindMissingWord(SentenceText):
            ...
            # Using the logits in an "argmax" function in order to retrieve the token_id, 
            # Representing the "Missing Word" that has been predicted.

            self.MissingWord = Tokenizer.decode(predicted_token_id) 
            # Using the Tokenizer to decode the predicted token id in order get the word that was missing in the form of text.
            ...
            return True 
            # Only used for ending the function. (The values from the function are being saved to class objects).
        ...
        def callback(instance):
            ...
            print('And the missing word is: ' + self.MissingWord)
            print(self.RecentlyCompleted)
...
if __name__ == '__main__':
    FindingWordo().run() 
    # Runs the application window according to the "MainAppClass" (Named FindingWordo) which has built and configured above.